            ...
            <div id='superContainer' class='primaryColor'>
                <section id='setupContainer' class='primaryColor center'>
                    ...
                    <section id='0' class='max-width left' style='display: block'>
                        ...
                        You are here because the Server Database ('<code>openServer</code>') was not found, but don't worry, here we will setup the Database!
                    </section>
                    <section id='1' class='max-width left' style='display: none'>
                        Here the process to prepare you Server:<br>
                        <ul>
                            <li>
                                In Linux run in the Terminal <code>sudo apt-get update && sudo apt-get upgrade -y && sudo apt-get install mariadb-server -y</code> to install <code>MariaDB</code> (if you already have <code>MySQL</code>)
                            </li>
                            <li>
                                Now install the SQL plugin for PHP with <code>sudo apt-get install php-mysql -y</code>
                            </li>
                            <li>
                                (Optional) Install the Zip and mbstring plugin for PHP with <code>sudo apt-get install php-files php-zip php-mbstring -y</code>
                            </li>
                            <li>
                                Enter the Database with <code>sudo mysql</code>
                            </li>
                            <li>
                                Create the Server Database <code>CREATE DATABASE 'chose_a_database_name';</code>
                            </li>
                            <li>
                                Now you will need to create 2 User for the Database, one for Reading only the Database (The SELECT type woll be fine) and antoher to Write and Delete elements (ALL PRIVILEGES is mandatory).<br>
                                Use this <a href='https://phoenixnap.com/kb/how-to-create-mariadb-user-grant-privileges' target='_blank'>link</a> to learn how to create them and granting the access.<br>
                                Tip: Instead of writing <code>*.*</code> when granting access, write <code>openServer.*</code>
                            </li>
                        </ul>
                        If you want, you can use only 1 User, but you need to grant him 'ALL PRIVILEGES'!<br><br>
                    ...
                    <section id='2' class='max-width left' style='display: none'>
                        ...
                        <form id='saveCredentials' method='POST' enctype='multipart/form-data'>
                            ...
                            Insert the name you choose for the Database: <input type='text' class='textInputs' name='databaseName' required><br><br>
