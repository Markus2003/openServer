...
<?php include $_SERVER["DOCUMENT_ROOT"] . '/src/include/super_top.inc.php' ?>
    ...
    <body>
        ...
        <div id='main'>
                    ...
                    if ( count( $folderList ) == 0 and count( $fileList ) == 0 )
                        ...
                        if ( count( $fileList ) > 0 )
                            foreach ( $fileList as $file )
                                if ( isReadableForServer( $file ) == 'Audio' ) {
                                    echo "
                                    <section class='primaryColor shadow'>
            ...
            <button type='button' id='uploadFile' class='button sensibleActionButton primaryColor shadow' onclick='uploadFile("Music", "<?php echo getInServerAddress( $_SERVER["PHP_SELF"] ) . $overrideFolder ?>")'><img src='/src/icons/upload.svg' />Upload a Music</button>
